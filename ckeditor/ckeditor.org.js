(function() {
					...
					if (d == "br") r.push(f);
						...
						for (;;) {
							...
							if (!f.isUnknown && !i.isUnknown && !h[d]) if (i.isOptionalClose) n.onTagClose(g);
								...
								i = g
							} else if (d in CKEDITOR.dtd.$listItem && !y(d, g)) n.onTagOpen(d == "li" ? "ul" : "dl", {}, 0, 1);
							else if (g in e && !y(d, g)) {
								!f.returnPoint && (f.returnPoint = i);
								i = i.parent
							} else {
	...
	(function() {
		...
		var o = /(?:&nbsp;|\xa0)$/,
			...
			l = CKEDITOR.tools.extend({}, i.$blockLimit, i.$block),
			q = {
				elements: {
					input: h,
					textarea: h
		...
		CKEDITOR.editable = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				insertElementIntoRange: function(a, b) {
					...
					var g, s;
					if (f) for (;
					(g = b.getCommonAncestor(0, 1)) && (s = CKEDITOR.dtd[g.getName()]) && (!s || !s[e]);) if (g.getName() in CKEDITOR.dtd.span) b.splitElement(g);
		...
		var n = function() {
				...
				return function(s, n, u) {
					...
					if (!A.checkReadOnly()) {
						...
						for (;
						(G = a(t.startContainer) && t.startContainer.getChild(t.startOffset - 1)) && a(G) && G.isBlockBoundary() && B.contains(G);) t.moveToPosition(G, CKEDITOR.POSITION_BEFORE_END);
					...
					if (p.equals(m)) {
						...
						if (w) if ((w = p.getParent()) && ((w.getDtd() || CKEDITOR.dtd.span)[f] || h) && (!g.parentRule || g.parentRule(w))) {
							...
							if (r == CKEDITOR.NODE_TEXT || t || r == CKEDITOR.NODE_ELEMENT && !p.getChildCount()) {
								for (var r = p, W;
								(j = !r.getNext(N)) && (W = r.getParent(), k[W.getName()]) && (W.getPosition(l) | J) == J && (!g.childRule || g.childRule(W));) r = W;
								q.setEndAfter(r)
							}
						} else j = true;
						else j = true;
			...
			enterBlock: function(b, d, a, h) {
				if (a = a || r(b)) {
					...
					if (i = a.splitBlock(l)) {
						...
						j = i.wasStartOfBlock;
						b = i.wasEndOfBlock;
						if (c) g = c.getParent(), g.is("li") && (c.breakParent(g), c.move(c.getNext(), 1));
	...
	(function() {
		...
		function P(a, c, d, b) {
			for (var e = function() {
				...
				}(), f = c.getDocumentPosition(), g = {}, i = {}, h = {}, j = {}, l = t.length; l--;) g[t[l]] = parseInt(e("border-" + t[l] + "-width"), 10) || 0, h[t[l]] = parseInt(e("padding-" + t[l]), 10) || 0, i[t[l]] = parseInt(e("margin-" + t[l]), 10) || 0;
			(!d || b) && I(a, b);
	...
	CKEDITOR.plugins.add("scayt", {
		...
		bindEvents: function(a) {
			...
			a.on("beforeCommandExec", function(c) {
				...
				} else if ("bold" === c.data.name || "italic" === c.data.name || "underline" === c.data.name || "strike" === c.data.name || "subscript" === c.data.name || "superscript" === c.data.name) if (c = a.scayt) c.removeMarkupInSelectionNode(), c.fire("startSpellCheck")
			});
	...
	CKEDITOR.plugins.scayt = {
		...
		loadScaytLibrary: function(a, d) {
			var c = this;
			"undefined" === typeof window.SCAYT || "function" !== typeof window.SCAYT.CKSCAYT ? (this.loadingHelper[a.name] = d, this.loadingHelper.loadOrder.push(a.name), CKEDITOR.scriptLoader.load(a.config.scayt_srcUrl, function() {
				var a;
		...
		CKEDITOR.plugins.add("stylescombo", {
			...
			init: function(c) {
				...
				c.on("stylesSet", function(b) {
					if (b = b.data.styles) {
						for (var a, h, d, e = 0, l = b.length; e < l; e++) if (a = b[e], !(c.blockless && a.element in CKEDITOR.dtd.$block) && (h = a.name, a = new CKEDITOR.style(a), !c.filter.customConfig || c.filter.check(a))) a._name = h, a._.enterMode = j.enterMode, a._.type = d = a.assignedTo || a.type, a._.weight = e + 1E3 * (d == CKEDITOR.STYLE_OBJECT ? 1 : d == CKEDITOR.STYLE_BLOCK ? 2 : 3), f[h] = a, i.push(a), k.push(a);
						i.sort(function(a, b) {
		...
		i.prototype = {
			onKeydown: function(a) {
				if (-1 < CKEDITOR.tools.indexOf(g, a.data.getKeystroke())) a.data.preventDefault();
					...
					if (e.isNavigationKey(a) || this.undoManager.keyGroupChanged(a)) if (b.strokesRecorded[0] || b.strokesRecorded[1]) b.save(!1, this.lastKeydownImage), b.resetType()
				}
			},
			onInput: function() {
